...
ElseIf InStr(1, url_str, "?albumId=", 1) > 0 or InStr(1, url_str, "&aid=", 1) > 0 Then
	...
	If InStr(album_id,"&ut=")>0 Then album_id=Mid(album_id,1,InStr(album_id,"&ut=")-1)
...
ElseIf InStr(html_str, "=[{id:") > 0 Then
	...
	'dmt:1250145227443,alc:true,comm:'',comdmt:1250145227443,comnum:0,kw:'',purl:''},
        ...
        Dim albumsINFO,temp(4),iCount,albumsID
...
On Error Resume Next
	...
	ElseIf InStr(url_str,"&ut=")>0 Then
		pass_word=urlencode(UTF8EncodeURI(urldecode(pass_word)))
		url_str=Mid(url_str,InStr(url_str,"&ut=")+4)
		return_ad_password_rules = "inet|10,13|http://photo.163.com/photo/dwrcross/" & user_id & "/u/" & user_id & "/dwr/call/plaincall/AlbumBean.getAlbumData.dwr?callCount=1&scriptSessionId=%24%7BscriptSessionId%7D822&c0-scriptName=AlbumBean&c0-methodName=getAlbumData&c0-id=0&c0-param0=number%3A" & album_id & "&c0-param1=string%3A" & pass_word & "&c0-param2=string%3Afromblog&c0-param3=number%3A" & url_str & "&c0-param4=boolean%3Afalse&batchId=4&ntime=" & CDbl(Now())
	End If
End Function
'------------------------------------------------------------------
