'2009-11-13 http://www.shanhaijing.net/163
...
url_str_split=split(mid(url_str,instr(lcase(url_str),"http://picasaweb.google.")+24),"/")
...
If photo_tf=1 Then
	return_download_url = "inet|10,13|http://picasaweb.google." & url_str_split(0) & "/" & url_str_split(1) & "/" & url_str_split(2)
Else
	return_download_url = "inet|10,13|http://picasaweb.google." & url_str_split(0) & "/" & url_str_split(1) & "/"
End If
...
If InStr(lcase(html_str), "_user.albums =[{id:'") > 0 Then
	...
	For i=0 to UBound(str_split)
		str_split(i)=Mid(str_split(i),InStr(lcase(str_split(i)),"',title:'")+9)
		'name
