'2009-06-19 http://www.shanhaijing.net/163
...
If InStr(url_str, "http://comic.92wy.com/go/show_") > 0 Then
	...
	return_download_url = "inet|10,13,34|http://comic.92wy.com/go/show_" & nameid & "1.htm|http://comic.92wy.com/"
	Else
	return_download_url = ""
	End If
    Else
...
ElseIf InStr(url_str, "http://comic.92wy.com/go/info_") > 0 Then
    ...
    url_str = Replace(url_str, ".htm", "")
    If IsNumeric(url_str) Then return_download_url="inet|10,13,34|http://comic.92wy.com/go/info_" & url_str & ".htm|http://comic.92wy.com/"
End If
End Function
'--------------------------------------------------------
...
url_temp=url_str
return_albums_list=""
If InStr(LCase(html_str), "<a href=http://comic.92wy.com/go/show_") > 0 Then
    Dim comic_Name
    comic_Name = Mid(html_str, InStr(LCase(html_str), "<h5>") +4)
    comic_Name = Mid(comic_Name, InStr(LCase(comic_Name), "<") +1)
    comic_Name = Mid(comic_Name, InStr(LCase(comic_Name), ">") +1)
    comic_Name = Trim(Mid(comic_Name, 1, InStr(LCase(comic_Name), "<") - 1))
    html_str = Mid(html_str, InStr(LCase(html_str), "<a href=http://comic.92wy.com/go/show_")+38)
    html_str = Mid(html_str, 1, InStr(LCase(html_str), "</p>")-1)
    
    album_list = Split(html_str, "<a href=http://comic.92wy.com/go/show_", -1, 1)

    For i = 0 To UBound(album_list)
        If InStr(LCase(album_list(i)), " target=_blank") > 0 Then
        return_albums_list = return_albums_list & "0||http://comic.92wy.com/go/show_" & Mid(album_list(i), 1, InStr( LCase(album_list(i))," target=_blank") - 1) & "|"
...
url_str = Mid(html_str, InStr(html_str, "ืÜ ") + 2)
page_counter = Mid(url_str, 1, InStr(url_str, " าณ") - 1)
If Not IsNumeric(page_counter) Then page_counter = 1
page_counter=Int(page_counter)
End If

If page <= page_counter and html_str<>"" Then
If InStr(1, LCase(html_str), "<img src=http://comicpic.92wy.com/main/", 1) > 0 Then
    html_str = Mid(html_str, InStr(LCase(html_str), "<acronym title=") + 15)
    comic_Name = replace(Trim(Mid(html_str, 1, InStr(LCase(html_str), ">") - 1))," ","_")
    
    html_str = Mid(html_str, InStr(LCase(html_str), "<img src=http://comicpic.92wy.com/main/") + 9)
    ...
    split_str = split_str & (page-unpic) & "าณ" & Mid(html_str, InStrrev(html_str,"."))
    comic_Name = comic_Name & "ตฺ" & split_str
...
End If
    ...
    If page < page_counter and html_str<>"" Then
        page = page + 1        
        return_download_list = return_download_list & page_counter & "|inet|10,13,34|http://comic.92wy.com/go/show_" & nameid & page & ".htm"
        Else
